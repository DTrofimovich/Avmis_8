...
float** multiplication_without_vectorization(float** mas1, float** mas2, int n1, int m1, int m2);
float** fast_multiplication(float**, float**, int, int, int);
...
void main()
{
	...
	time_begin = clock();
	mas3 = multiplication_without_vectorization(mas1, mas2, n1, m1, m2);
	time_end = clock();
	printf("simple: %lf sec\n", (double)(time_end - time_begin)/CLOCKS_PER_SEC);
    outputmatrix(mas3, n1, m2, "simple");

	time_begin = clock();
	mas3 = fast_multiplication(mas1, mas2, n1, m1, m2);
	time_end = clock();
...
float** fast_multiplication(float** mas1, float** mas2, int n1, int m1, int m2)
{
	int i, j, k;
	float f, *_mas3, *_mas2;
	float** mas3 = allocation(n1, m2);
